...
describe('uploading the same device twice', function () {
    ...
    before(async () => {
        ...
        const fileDataStub = Object.assign(
            {
                add: (filename, data, cb) => {
                    cb(null, { key: 'k', href: 'h' });
                },
                addOrReplace: (id, filename, data, cb) => {
                    cb(null, { key: id, href: 'h' });
                },
                findBy: (id, cb) => {
                    cb(Object.assign(new Error('NoSuchKey'), { name: 'NoSuchKey' }));
                },
                removeBy: (id, cb) => {
                    cb();
                },
                generateUri: (key) => `http://example/${key}`,
            },
            { '@global': true }
        );
    ...
    const upload = () =>
        new Promise((resolve) => {
            ...
            manager.add(
                'pd-1',
                'sn-1',
                { lastUpdate: 'L', components: [{ componentName: 't', measurements: {} }] },
                (err) => {
                    clearTimeout(timer);
                    resolve({ fired: true, err: err });
                }
            );
        });

    it('accepts the first upload', async () => {
        const result = await upload();
        expect(result.fired).to.equal(true);
        expect(result.err || null).to.equal(null);
    });

    it('accepts a second upload of the same device, and answers it', async () => {
        await upload();
        const result = await upload();
