...
describe('DynamoDB repository (round trip against DynamoDB Local)', function () {
    ...
    describe('a table with a composite key', () => {
        it('returns what was written, by both key parts', async () => {
            await promised((cb) =>
                devices.addOrReplace({ productDescriptorId: 'pd-1', serialNumber: 'sn-1', spl: 42 }, cb)
            );
        ...
        it('does not return an item whose range key differs', async () => {
            await promised((cb) =>
                devices.addOrReplace({ productDescriptorId: 'pd-2', serialNumber: 'sn-a', spl: 1 }, cb)
            );
            const found = await promised((cb) => devices.findBy('pd-2', 'sn-b', cb));
            expect(found).to.equal(undefined);
        });

        it('removes only the addressed item', async () => {
            await promised((cb) =>
                devices.addOrReplace({ productDescriptorId: 'pd-3', serialNumber: 'keep', spl: 1 }, cb)
            );
            await promised((cb) =>
                devices.addOrReplace({ productDescriptorId: 'pd-3', serialNumber: 'drop', spl: 2 }, cb)
            );
        ...
        it('drops an undefined attribute instead of writing null or failing', async () => {
            await promised((cb) =>
                devices.addOrReplace(
                    { productDescriptorId: 'pd-4', serialNumber: 'sn-4', spl: 7, absent: undefined },
                    cb
                )
            );
    ...
    describe('a table with a hash key only', () => {
        it('round-trips and removes without a range key', async () => {
        ...
        it('returns undefined for an item that was never written', async () => {
            expect(await promised((cb) => expiries.findBy('never-written', cb))).to.equal(undefined);
        });
    });

    describe('querying a secondary index by equality', () => {
        before(async () => {
            await promised((cb) =>
                descriptors.addOrReplace({ id: 'd-1', vendor: 'linn', productType: 'akubarik' }, cb)
            );
            await promised((cb) =>
                descriptors.addOrReplace({ id: 'd-2', vendor: 'linn', productType: 'akubarik' }, cb)
            );
            await promised((cb) =>
                descriptors.addOrReplace({ id: 'd-3', vendor: 'linn', productType: 'akudorik' }, cb)
            );
            await promised((cb) =>
                descriptors.addOrReplace({ id: 'd-4', vendor: 'other', productType: 'akubarik' }, cb)
            );
        });

        it('returns every item matching both key attributes', async () => {
            const items = await promised((cb) =>
                descriptors.queryByEquality(
                    { indexName: DESCRIPTORS_INDEX, equals: { vendor: 'linn', productType: 'akubarik' } },
                    cb
                )
            );
        ...
        it('excludes items matching only the partition key', async () => {
            const items = await promised((cb) =>
                descriptors.queryByEquality(
                    { indexName: DESCRIPTORS_INDEX, equals: { vendor: 'linn', productType: 'akudorik' } },
                    cb
                )
            );
        ...
        it('returns nothing when no item matches', async () => {
            const items = await promised((cb) =>
                descriptors.queryByEquality(
                    { indexName: DESCRIPTORS_INDEX, equals: { vendor: 'linn', productType: 'nonesuch' } },
                    cb
                )
            );
        ...
        it('queries the base table when no index is named', async () => {
            const items = await promised((cb) => descriptors.queryByEquality({ equals: { id: 'd-3' } }, cb));
            expect(items.map((i) => i.id)).to.deep.equal(['d-3']);
        });
    });

    describe('a result set larger than one DynamoDB page', () => {
        ...
        before(async function () {
            ...
            for (let i = 0; i < COUNT; i++) {
                await promised((cb) =>
                    devices.addOrReplace(
                        { productDescriptorId: 'paged', serialNumber: `sn-${String(i).padStart(4, '0')}`, blob: BIG },
                        cb
                    )
                );
            }
        ...
        it('returns every matching item, not just the first page', async () => {
            const items = await promised((cb) =>
                devices.queryByEquality({ equals: { productDescriptorId: 'paged' } }, cb)
            );
            expect(items.length).to.equal(COUNT);
        });

        it('returns them without duplicates', async () => {
            const items = await promised((cb) =>
                devices.queryByEquality({ equals: { productDescriptorId: 'paged' } }, cb)
            );
    ...
    describe('callback arity, which async.waterfall depends on', () => {
        ...
        it('calls back from a delete with an error slot and nothing else', (done) => {
            devices.addOrReplace({ productDescriptorId: 'arity', serialNumber: 'sn' }, () => {
    ...
    describe('a caller contract violation is reported, not thrown', () => {
        ...
        it('reports an absent match set through the callback instead of throwing', async () => {
            let raised;
            try {
        ...
        it('refuses a range value on a table that has no range key', () => {
            ...
            expect(() => {
                descriptors.findBy('id-1', 'unexpected-range', () => {});
            }).to.throw(/no range key/);
        });
    });

    describe('failures reach the caller', () => {
        it('passes a DynamoDB error to the callback rather than throwing', async () => {
