...
module.exports = function DynamoDbRepository(awsRegion, tableName, hashKey, rangeKey) {
    ...
    function findBy(id, rangeOrCallback, maybeCallback) {
        ...
        adapt(client.send(new GetCommand({ TableName: tableName, Key: key })), (result) => result.Item, callback);
    }

    function addOrReplace(item, callback) {
        adapt(client.send(new PutCommand({ TableName: tableName, Item: item })), () => undefined, callback);
    }
